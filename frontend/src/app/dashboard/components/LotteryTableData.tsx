...
export const columns: ColumnDef<Payment>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("lottery")}</div>
    ),
  },
  {
    accessorKey: "bet",
    header: "Bet",
    cell: ({ row }) => <div className="lowercase">{row.getValue("bet")}</div>,
  },
  {
    accessorKey: "method",
    header: "ประเภท",
    ...
    <div className="w-full">
      <div className="grid grid-cols-10 pb-4">
        <div className="col-start-2 col-span-3">
            ...
            value={(table.getColumn("lottery")?.getFilterValue() as string) ?? ""}
            onChange={(event) =>
                table.getColumn("lottery")?.setFilterValue(event.target.value)
            }
